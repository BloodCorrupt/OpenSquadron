...
/**
 * Walks a saved InstagramBotFlow and dispatches its actions through the Instagram API.
 *
 * Supports graph format:      { format:'graph', nodes:[...], edges:[...] }
 */
class InstagramBotFlowExecutor
{
    ...
    public function execute(InstagramBotFlow $flow, Subscriber $subscriber, ?string $startNodeId = null, ?string $commentId = null): void
    {
        $data = $flow->getFlowData();
        $connection = $flow->getInstagramConnection();
     ...
     */
    private function runGraph(InstagramBotFlow $flow, array $graph, Subscriber $subscriber, ?\App\Entity\InstagramConnection $connection = null, ?string $startNodeId = null, ?string $commentId = null): void
    {
        $nodes = [];
        ...
        while ($current !== null && $steps < self::MAX_STEPS) {
            ...
            if (($node['type'] ?? '') === 'user_input') {
                ...
                if ($varName !== '' && $question !== '' && !empty($psid)) {
                    $response = $this->Instagram->sendMessage($psid, $question, $connection);
     ...
     */
    private function runAction(array $action, Subscriber $subscriber, ?\App\Entity\InstagramConnection $connection = null, ?string &$commentId = null): void
    {
        $type = (string)($action['type'] ?? '');
        $psid = (string)$subscriber->getPsid();

        if (empty($psid)) {
            $this->logger?->warning('InstagramBotFlowExecutor: Subscriber has no PSID');
            return;
        }
        ...
        try {
            switch ($type) {
                ...
                case 'send_text':
                    ...
                    if ($commentId !== null) {
                        $response = $this->Instagram->privateReplyToComment($commentId, $text, $connection);
                        $commentId = null;
                    } else {
                        $response = $this->Instagram->sendMessage($psid, $text, $connection);
                    }
                ...
                case 'send_image':
                    ...
                    $response = $this->Instagram->sendMediaMessage($psid, 'image', $url, $connection);
                    ...
                    if ($caption !== '') {
                        ...
                        $this->Instagram->sendMessage($psid, $caption, $connection);
                    }
            ...
            $this->logger?->warning('InstagramBotFlowExecutor action failed', [
                'type' => $type,
                'error' => $e->getMessage(),
            ]);
        }
